"""Native cognis-connect emit for approvewarden — forward findings to any platform.

Maps approvewarden's JSON output to the canonical `Finding` and forwards it via
...
Usage:
    approvewarden ... --format json | approvewarden-emit --to stix
    approvewarden-emit --to slack --url $WEBHOOK --dry-run < findings.json
"""
...
SOURCE = "approvewarden"


def map_record(rec: dict) -> dict:
    """Tailor a approvewarden record toward the Finding contract before normalize().
