...
```mermaid
flowchart LR
  IN[addresses + transactions] --> P[approvewarden<br/>cluster + trace]
...
`approvewarden` is interoperable with every popular way of using AI:

- **MCP server** — `approvewarden mcp` (Claude Desktop, Cursor, Cognis.Studio, [uncensored-fleet](https://github.com/cognis-digital/uncensored-fleet))
- **OpenAI-compatible / JSON** — pipe `approvewarden scan . --format json` into any agent or LLM
...
```bash
pip install "git+https://github.com/cognis-digital/approvewarden.git"    # pip (works today)
...
docker run --rm ghcr.io/cognis-digital/approvewarden:latest --help        # Docker
brew install cognis-digital/tap/approvewarden                             # Homebrew tap
curl -fsSL https://raw.githubusercontent.com/cognis-digital/approvewarden/main/install.sh | sh
```
